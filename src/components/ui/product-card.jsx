    ...
    >
      <Link href={`/products/${product.id}`} className="block h-full">
        <div className="aspect-[4/3] overflow-hidden relative">
          ...
          <div className="absolute top-2 right-2 md:top-3 md:right-3 bg-white/95 backdrop-blur-sm px-2 py-0.5 md:px-2.5 md:py-1 rounded-full text-[8px] md:text-[10px] font-bold text-primary uppercase tracking-wide shadow-sm">
            {product.category}
          </div>
        </div>
        <div className="p-3 md:p-5">
          <div className="text-accent text-[8px] md:text-[10px] font-semibold mb-1 md:mb-1.5 uppercase tracking-wider">
            {product.category}
          </div>
          <h3 className="text-sm md:text-lg font-display font-bold text-foreground mb-1 md:mb-2 group-hover:text-primary transition-colors line-clamp-1">
            {product.title}
          </h3>
          <p className="text-muted-foreground mb-2 md:mb-3 line-clamp-2 text-[10px] md:text-sm leading-relaxed">
            {product.description}
          </p>
          <div className="flex items-center gap-2">
            ...
            <div 
              className="p-1 md:p-1.5 rounded-full border border-primary/20 hover:border-primary hover:bg-primary/5 transition-all duration-300 group/icon"
            >
              <ArrowRight className="w-3 h-3 md:w-3.5 md:h-3.5 text-primary group-hover/icon:translate-x-0.5 transition-transform" />
            </div>
          </div>
        </div>
      </Link>
